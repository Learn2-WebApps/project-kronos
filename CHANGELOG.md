...
## [v4.4.28] - 2026-05-19
### Fixed
- 인터뷰 화면 남은 질문 횟수 UI 가림 문제 해결 (헤더 + 입력창 위 이중 표시, z-index 30)
- 초기 단서 "배신" 표현이 특정 인물 지목 유도하던 문제 수정 → "억울한 혐의"로 변경

### Added
- 티타니아 접점 단서 F-33 (한지훈), F-34 (윤서경), F-35 (강혜린) - 의혹 수준 노출
- 캐릭터별 "정보 관리 매트릭스" 명시 (허용/금지 정보 구분)
- ECHO 오세라 브리핑에 티타니아 유출 의혹 상세 추가 및 티타니아 질문 응답 지침

### Changed
- 캐릭터 응답 길이 200자 → 250~280자로 확장 (자연스러운 대화)
- 응답 스타일: 부인 + 관련 정보 흘리기 조합 방식
- 총 단서 수 51 → 54

### Maintained (변경 없음 - 난이도 유지)
- 정민호-오세라 4단계 방어선 시스템 (v4.4.26)
- 공모 사실, 알리바이 조작, 프레임업 주범 등 핵심 진실은 학습자 추론 영역으로 유지
- KEY-4 도출 조건 유지

## [v4.4.25] - 2026-05-19
### Added
- 분산형 연결 단서 5종 (F-28~F-32): 정민호-오세라 공모 추론을 자연스럽게 유도
- 인터뷰 화면 상단에 남은 질문 횟수 배지 (색상 경고 포함: 3회 노란색, 1회 빨간색)
- 인벤토리 "전체" 탭 (기본 선택, 수집 순서 정렬)
### Changed
- 인벤토리 모달에서 KEY 단서 강조 제거 (시각적 단순화)
- KEY-4 유도 조건 확장 (F-26 + F-27 + [F-28/F-31] + [F-29/F-30] 조합)
- 총 단서 수 46 → 51
- 오세라 방어 톤: 특정 키워드(정민호, 컨퍼런스) 동시 언급 또는 2회 압박 시 F-31 노출 지침 추가

## [v4.4.24] - 2026-05-19
### Changed
- 5명 캐릭터 이미지를 누끼 작업 완성본으로 교체 (한지훈, 정민호, 강혜린, 오세라, 윤서경)
- 파일 용량 대폭 감소(평균 7MB → 0.6MB)로 페이지 로딩 속도 개선
- jung-minho-angry.png 최신본 반영
### Added
- 이미지 프리로딩 시스템 (홈, 인터뷰, 결과 진입 시점)
- 캐릭터 표정 동적 전환 (LLM <emotion> 태그 기반)
- 한지훈 smile 표정 활용 (협조적 분위기)
- "사건의 전말" 시나리오 화면 (정답 공개 후 자동 재생)
  - 정민호 angry 이미지 배경, 6단락 자동 진행 (페이드 효과)
  - localStorage 기반 시나리오 중복 재생 방지
- Emotion 타입에 'smile', 'angry' 추가 (angry는 시나리오 전용)

## [v4.4.23] - 2026-05-19
### Added
- ECHO 인물별 상세 브리핑 템플릿 (오세라, 강혜린, 윤서경, 정민호)
- 각 200자 내외, 동일한 중립성 문구로 마무리
- 초기 브리핑 끝에 "이름을 말씀해 주시면 자세히 안내" 문구

### Notes
- 강혜린 브리핑은 윤서경 친분(맥거핀)을 의도적으로 생략
- 정민호 브리핑은 시간선 단서(사건 당일 식사) 제외, 표면 신뢰 관계만 포함

## [v4.4.22] - 2026-05-19
### Fixed (긴급)
- ECHO 모달이 모든 페이지를 가로채던 치명적 버그 수정
- isModalOpen 상태를 persist에서 제외 (새로고침 시 모달 자동 재개 방지)
- /entry, /admin 경로에서 ECHO/Inventory 버튼·모달 렌더링 차단 (경로 가드 강화)
- ESC 키로 ECHO 모달 닫기 보장
- 자동 브리핑 트리거에 pathname·세션·useRef 다중 가드 추가
- ECHO 시스템 프롬프트 4인 동등성 규칙 최상단 명시
- 하드코딩된 "정민호 팀장" 표현 일괄 점검 및 중립 표현으로 교체 (편향 제거)
